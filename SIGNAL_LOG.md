...
- **Date:** 2026-09-08
- **Category:** discovery, fix
- **Description:** Took the finding from Claim #011 -- that a dispatch and the pieces it dispatches to are different tests -- to the three dispatches a *user* touches, and the third one was broken in front of the owner's second priority. **Keyboard input had one test, for F1 to F3**: thirteen bindings and ten branches, so every movement key, the fly toggle, chat focus, centring on the avatar, and the orbit and map cameras' zoom, rotate, pan and lift were covered by nothing, while `AgentControlFlags`, `Camera3D.orbit_zoom` and `Bus.dispatch` all had tests of their own. Twenty mutants die now, and three of the tests are not obvious: a missing `RemoveControlFlags` does not stop something happening, it stops it *stopping*; Shift+PageUp means two things and a leaked flag would fly the avatar every time somebody raised the camera; and Return has to consume the key or focusing chat also walks. **The HUD had the crash half and only the crash half** -- `HUDControlSweepTests` presses every control and asserts none of them raises, and says in its own docstring that it asserts nothing about what they do -- so forty-odd `event.ui_element is self.<x>` branches were reached with their effects unchecked; forty-three now have their effect pinned, including the six inspector buttons that are the GUI end of C, D and E and had stood uncovered because reaching them end to end needs a window and a sim together. Three behaviours surfaced in the writing: the water slider runs 0--100 while the setting runs 0--1 (wrong by a factor of a hundred, and the clamp would have hidden it), re-choosing the current render mode is a deliberate no-op because rebuilding throws away every uploaded mesh, and a double click on an inventory row must select *and* open. **The third dispatch was the login screen, and it did not work**: `UIDropDownMenu.selected_option` is a `(text, object id)` pair in the pinned pygame_gui and was a bare string in older ones, so every `preset == "Second Life"` was false and choosing Second Life logged in to `http://127.0.0.1:9000/` -- silently, because the local sim answered -- with the URI field hidden and disabled unless the preset is Custom, so it could not be corrected by hand either. The same branch filled `last` where `./gui.sh sl` fills `home`. **Both existing tests passed on both bugs**, because both hand-built the state the code was looking for: a bare-string `selected_option`, and a legacy `pygame.USEREVENT` carrying `user_type`. Neither is a value the library produces. That is a fourth shape of a test that cannot fail -- **a check that supplies the state the code wants is a check that the code agrees with itself** -- and the replacements click the control and read shapes off a live widget; eight mutations of the preset routing, button handling and credential check die against them. One self-correction recorded in the source rather than quietly dropped: I first concluded the legacy event path was dead code, and a real-click probe showed pygame_gui 0.6.14 posts *both* forms, so it worked; the bugs were the pair and the start location. Also found that any test building a `LoginScreen` from the repository root loaded the owner's **real stored credentials** into the fields and, because the constructor auto-connects on a complete profile, tried to log in with them. Separately, **`--camera-sweep` moves the camera reported to the simulator for interest management and not the one that draws**, which cost a soak run to learn: run 7 was started in that belief and came back with the same `scene.rebuilt_frames` -- 10, across two hours -- as run 6. `--camera-orbit-seconds` turns the render camera, has to be applied *after* the camera preset (the default preset rewrites the yaw every frame, and a run with the orbit overwritten still finishes and still reports healthy), and `render.camera_yaw` is now logged as the control for every leak gauge, because `flat` otherwise means either "nothing leaked" or "nothing was asked of it" and there was no way to tell those apart. Suite 2,720 -> 2,785.
- **Endorsements:**
...
- **Date:** 2026-09-08
- **Category:** discovery, fix
- **Description:** The XML-RPC login in this client had never run over TLS, and the reason is the same one as Claim #012's: nothing tested the wiring. `xmlrpc.client.ServerProxy` selects `SafeTransport` for an `https` URI **only when it is not handed a transport of its own**; this client always hands it one, because the login response needs a socket timeout and a size bound, and that transport subclassed the plain `Transport` whatever the scheme. So `https://login.agni.lindenlab.com/cgi-bin/login.cgi` -- priority B's grid -- opened an **unencrypted** connection, and not to 443 either: `HTTPConnection(host)` defaults the port to 80, so the port the URI implies is dropped along with the scheme. Every `https` login this client could have made would have put an account's password hash on the wire in clear, to the wrong port. It sat there because the sim this project is developed against is `http://127.0.0.1:9000/`, so the one scheme in daily use was the working one, and because **every existing test of `_login_sync` replaced `ServerProxy` wholesale** -- the transport was constructed and discarded unexercised in all of them, which is the exact shape of the finding from Claims #010 through #012 landing on the encryption. `transport_for` now picks by scheme with the timeout and the response bound in a mixin so neither transport can lack one, and `LiveTLSLoginTests` stands up an XML-RPC server on the loopback with its own certificate and logs in to it over `https`, so the claim is the whole chain rather than the type of an object; two of its three cases are the failure modes -- a plain HTTP request to the TLS port must fail rather than be served, and an untrusted certificate must arrive as a `LoginError` and not a traceback. Six mutations of the scheme choice, the context, the timeout and the bound die against it; a seventh survived and was the right answer to a wrong question -- `urlsplit` lowercases the scheme itself, so the `.lower()` I had added was dead code, and the test that seemed to cover it now pins the stdlib behaviour it actually depends on instead. **No flag turns verification off**: an OpenSim grid behind a private CA is a real case and the answer is `LoginClient(ssl_context=ssl.create_default_context(cafile=...))`, because the payload of this request is a password. Checked and clear: the other eleven HTTP calls in the tree -- every capability client and the event queue -- go through `urllib.request.urlopen`, which takes TLS from the scheme by itself, and nothing else supplies a transport. Also made a refused login say the grid's `reason` beside its `message`, with deliberately no table of reason codes and no branching on them, since this client has never completed a login against Second Life and a table of that grid's refusals would be one nobody here has seen. Left alone on purpose and written down instead: the payload sends an empty `mac` and `id0`, and filling those in means fingerprinting the owner's machine for a third party, which is the owner's call. Suite 2,785 -> 2,795 -> 2,801.
- **Endorsements:**
...
- **Date:** 2026-09-08
- **Category:** discovery, fix
- **Description:** Four more dispatches and two crash-class sweeps, and the two findings that mattered most were both about a command or a packet that failed *quietly*. **The CLI's subcommand chain exited zero when it broke**: `main` is thirteen `if args.command == "..."` comparisons, `build_parser` registers thirteen subcommands, nothing made the two agree, and a name renamed on one side fell off the end of the chain into the status line -- which prints a friendly sentence and returns 0. `sync-object` doing nothing and reporting success is the worst answer available on the one command priorities C, D and E run through. Falling off the end now names the command on stderr and exits 2, and the coverage test compares a live parser against branch names read out of the source so a fourteenth subcommand is covered on the day it is written. **And then the documented way to run that command turned out not to exist**: `./run.sh tester sync-object --object <uuid> --folder ./work --push` appears four times in the handoff and answers `Unknown command: sync-object`, because the launcher arm was never written and the feature was only ever reachable as `python -m vibestorm.app.cli sync-object`, which is how it was live-verified. A missing arm looks exactly like a command that was never meant to exist. The two lists are a hand-maintained translation in two languages -- `census` runs `world-census`, `eventq` runs `event-queue-once` -- and are now compared both ways; a mutant earned the word "reachable" in that test, because the first version scanned the whole file for `vibestorm.app.cli <name>` and passed with the `census)` arm deleted and `do_census` left orphaned. **`_wire_scene` had no tests at all** -- nineteen `bus.subscribe` lines in the 3D viewer, seven in the 2D one, two separate copies of a function with the same name; thirty-two mutants die now, including three handlers pointed at a neighbour and the render tile cache no longer cleared on a region change. Then the fuzz work: the receive-loop sweep asked what a decoder *raises*, and this asked what it **returns**. A terrain patch header carries a 32-bit IEEE float straight off the wire as the DC offset and every height is `coefficient * mult + dc_offset`; of twenty thousand random payloads, ninety-one decoded and 1,280 heights came back non-finite, every one from a NaN offset. Not a crash, which is worth being exact about: `apply_patch` writes into the heightmap the **session** keeps, so `height_at` answers NaN over that 16x16 metre square from then on, the terrain mesh draws nothing there, and `eye_clear_of_the_ground` quietly stops working -- every comparison against a NaN is false, so the camera is never blocked by that hill and is left standing inside it. One packet, and a piece of the ground is gone and the camera walks through it for the rest of the session, with nothing raised and nothing logged. Guarded on the header rather than on the heights, and that is earned rather than asserted by a sweep of the fields that feed the arithmetic; afterwards the same twenty thousand payloads produce no non-finite height and eighty-nine of the ninety-one still decode. Seven other decoders can return one and each was followed to where it lands rather than guarded by reflex -- position and scale are already refused at the scene boundary by `is_a_place` and `_is_a_size`; the ExtraParams family, the texture animation and the texture entry's per-face numbers reach the HUD inspector's text and nothing else, since the renderer reads only `texture_for_face`, which is a UUID -- and that review is now a list the sweep fails against when an eighth joins. Two things the fuzzing needed before it meant anything. **A corpus has to get past the front door**: a terrain payload's group header names a patch size and every size but 16 and 32 is refused before a patch header is read, so the decoder that reads a float per patch was being swept without ever reading one, and removing the new guard did not fail the sweep until two corpus shapes with a real group header were added. And the mesh's bounding-box guards -- `PositionDomain` and `TexCoord0Domain`, where a NaN corner is not one bad vertex but every vertex, since each is a `u16` scaled between the corners -- survived the asset mutation corpus untouched, because flipping bytes rarely lands on the eight that make up a corner; they are pinned directly now, and the first version of that test named the field `TexCoordDomain` and passed while testing nothing, which the mutation run is what caught. Separately, **the soak instrument stopped crying wolf about the gauge that found its only real leak**: `udp.seen_sequences` climbs towards the 8,192 its two-window memory is bounded by, and read `growing` on every run. `RecentSequences.capacity` states the bound once -- it had been in a docstring, a comment and an assertion and nowhere in the code -- the log records it as `udp.seen_sequences.limit`, and `growth_report` reads any `<name>.limit` row as that row's ceiling: under it `bounded`, over it `over-bound` ahead of every other rule, read off the **peak** rather than the last sample, because a bound exceeded once and recovered is still a bound that does not hold. Suite 2,801 -> 2,862.
- **Endorsements:**
