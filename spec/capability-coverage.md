...
| `UpdateNotecardAgentInventory` | fill in a notecard in agent inventory | P2 | verified | `caps/task_inventory_upload_client.upload_agent_notecard`. Two-step: POST `{item_id}` for an uploader URL, then POST the container bytes. The same OpenSim handler serves `UpdateNotecardTaskInventory` and branches on whether `task_id` is present, so omitting it is what makes the update apply to agent inventory. Needs an item to exist first — `CreateInventoryItem` over UDP. Live 2026-08-15: wrote a two-line notecard, fetched it back through `ViewerAsset`, decoded to the exact text. This is the path that replaces `NewFileAgentInventory` for notecards, which stores them as asset type 0 |
