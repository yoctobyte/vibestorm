...
| Environment | A day cycle's `moon_id`, `cloud_id` and water `normal_map` are ordinary texture assets behind the ordinary `GetTexture` capability, served by OpenSim's own asset service -- not viewer-install files. Nothing in the document says so, and a client that reads only the numbers invents a moon, a cloud layer and a wave shape it could have downloaded (verified live 2026-09-06: all three fetch, as do `bloom_id`, `halo_id` and `rainbow_id`) | `world/environment.py` |
| Environment | `sun_id` is present in all eight sky keyframes of OpenSim's default cycle and is the **null UUID** in every one. That is the document writing "no texture", not a field it forgot -- a client that treats a present id as a fetchable one waits forever for an answer that cannot come | `world/environment.py` |
| Environment | `cloud_scale` is a scale for the `cloud_id` *texture*, and the texture tiles seamlessly (measured: mean |difference| between its left and right edge columns is 1.1 against 48 for an arbitrary pair). Nothing says either. A client that reads the number as a noise cell size and substitutes its own field draws a sky an order of magnitude too fine | `viewer3d/atmosphere.py` |
| Environment | `cloud_pos_density1` and `cloud_pos_density2` are (x, y, density) each, and the two x/y pairs are **identical in all eight keyframes** of OpenSim's default cycle. So the two layers start in the same place and the sky is one field at the sum of two densities; no captured region can distinguish a client that reads the second pair from one that reads the first twice | `viewer3d/atmosphere.py` |
| Environment | The water frame's `normal_map` is a *normal* map, not a height or gradient map: its blue channel runs 234-255, so the stored vector is the surface normal and its horizontal part is **added** to the vertical. A reader that carries over the minus sign a height field needs turns every crest into a trough -- which from above is the same sea, and from the waterline is the opposite one | `viewer3d/perspective.py` |
| Environment | `normal_scale` scales the `normal_map` *texture*, and one tile of that texture is not one wave. The map OpenSim's default cycle names holds about **nine ripples across its tile** (measured: the power spectrum of its red channel peaks at nine cycles, with a broadband tail out to the pixel). Nothing in the document says how many, so a client that lays a wavelength to a tile draws a sea at a tenth the scale, which mip-maps to a flat sheet rather than failing visibly | `viewer3d/perspective.py` |
| Environment | Nothing in the water frame says which way the `normal_map`'s green channel points -- the OpenGL/DirectX handedness question -- and the map OpenSim's default cycle names is close enough to symmetric across its crests that no capture of it can settle the question either | `viewer3d/perspective.py` |
| Environment | A water keyframe carries a fourth asset id, `transparent_texture`, and it is not a transparency mask: OpenSim's default cycle names `2bfd3884-7e27-69b9-ba3a-3e673f680004`, which fetches through the ordinary `GetTexture` capability as a 256x256 **opaque** blue sheet with no alpha channel at all, uncorrelated with the `normal_map` beside it (verified live 2026-09-06). The name is the whole trap: a reader that treats it as a mask draws nothing and cannot tell that from a field it did not need | `world/environment.py` |
| Environment | A sky keyframe's `moon_rotation` and `sun_rotation` are **quaternions**, not directions, and the extra three components are load-bearing for the moon: the rotation that says where the moon is also says which of its own axes is across its face and which is up. A reader that takes only the direction has to manufacture a pair of axes, and the obvious pair -- the cross product with world up -- reverses at the meridian, turning the face a half circle in one frame every night | `viewer3d/atmosphere.py` |
| Neighbouring regions | A child circuit that does everything the wire asks -- `UseCircuitCode`, `RegionHandshakeReply`, acks, `AgentUpdate` -- receives **no terrain at all**. The unlock is not on the wire: OpenSim's `ScenePresence.SendInitialData` returns early unless both `m_gotRegionHandShake` and `Caps.CapsFlags.SentSeeds` are set, and `SentSeeds` is set at the end of `BunchOfCaps.SeedCapRequest` -- the handler for an HTTP POST to the seed capability the `EstablishAgentCommunication` event carries. Measured: 0 terrain patches without the POST, 256 with, on an otherwise identical circuit. A client that reads `EnableSimulator` as the whole story gets a region name, a water height and a trickle of cloud, and nothing tells it what is missing | `udp/neighbour.py` |
| Neighbouring regions | A child circuit **does** receive the region's objects -- `ObjectUpdate`, `ObjectUpdateCompressed` and `ObjectUpdateCached` all arrive for an agent who is only looking. Nothing says so, and the quiet failure is the opposite of the loud one: a client that assumes a child circuit is for terrain draws neighbouring regions as empty hillsides and has no reason to suspect the data was already being sent to it | `udp/neighbour.py` |
| Neighbouring regions | Local ids are assigned **per region**, so object 42 in the region next door and object 42 underfoot are different prims. A client that merges two regions' object updates into one dictionary keyed by local id silently loses one of them, and any cache keyed the same way -- a packed model matrix, a decoded shape -- hands one prim the other's data. Nothing on the wire distinguishes them; only the circuit a packet arrived on does | `udp/neighbour.py` |
| Neighbouring regions | Water height is announced **per region**, in each region's own `RegionHandshake`, and a child circuit's handshake carries that region's level rather than the root's. A viewer that draws one sea plane at the region-it-is-standing-in's level draws it over its neighbours too: where the neighbour's sea is lower, ours runs up its beach, and a region a few metres below ours is drawn looking sunk. Nothing on the wire objects, and the seam only appears at a border between two regions that were configured differently | `viewer3d/perspective.py` |
