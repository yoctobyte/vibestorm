...
| `RebakeAvatarTextures` | the simulator cannot find a baked avatar texture | P2 | handled | the sim sends this when an `AgentSetAppearance` named a bake whose asset is not in its store, and until something answers it the avatar is a cloud to everyone else. There is no rasterizer here to bake with, so the answer is to re-assert the appearance already held under a higher serial -- the fix when the asset exists and only the sim's cache entry went missing. 45 arrived over the recorded sessions and were ignored |
...
| `KillObject` | remove object from world cache | P1 | verified | observed on 2026-09-06 by linking two rezzed prims and taking the root away: the simulator sent one `KillObject` naming the root and **not** the child, which then sat in the world view for the rest of the session. A killed object's descendants now go with it -- except a seated avatar, which stands up rather than ceasing to exist |
| `ObjectProperties` | an object's full properties | P2 | verified | the long form of the family reply, sent for an object that has been *selected*: everything the family carries plus the creator, the creation date, the inventory serial, and where a rezzed object came from in inventory. Selected a prim this client had just rezzed on local OpenSim 2026-09-06 and got back its own agent id as both creator and owner, a serial of 0, and a name the family reply agreed with -- `tools/verify_object_properties.py`. **`CreationDate` is microseconds** since the epoch, which the template does not say: the prim came back as 1788649776000000, the instant it was rezzed |
...
| `ObjectDelete` | remove prims from a region | P4 | handled | built and sent by `tools/delete_prims.py`; **the local OpenSim build has no handler for it** -- its log answers every attempt with `ignoring unhandled packet ObjectDelete`, so nothing this project sends has ever deleted anything. Prims that vanished around the first runs vanished on their own. The encoder follows the template; SL is expected to implement it, and nothing has confirmed that |
| `ObjectDelink` | break prims out of a linkset | P4 | tested | reaches OpenSim's handler and unlinked a real child on 2026-09-05. A duplicate local id in the list made the simulator throw a `NullReferenceException` inside `SceneGraph.DelinkObjects`; the tool deduplicates now |
| `AgentRequestSit` / `AgentSit` | sit on an object | P4 | verified | the two-step sit: request, then commit once `AvatarSitResponse` names the seat. Sat on a rezzed prim on local OpenSim 2026-09-05, which reparented the avatar onto it -- see `tools/verify_seated_avatar.py` |
| `ObjectAttach` / `ObjectDetach` | wear an in-world prim, and take it off | P4 | verified | attached a rezzed prim to the test avatar on local OpenSim 2026-09-06 and took it off again, twice. Attaching reparents the prim onto the avatar and its position becomes the avatar's frame -- the same shape as a linkset child and a seated avatar, which is what `viewer3d/linkset.py` claimed and had not watched. `tools/verify_attachment_frame.py`. Detaching takes the prim *into inventory*, so it leaves the region: that is the only working way this project has found to get a prim out of a region whose `ObjectDelete` is unhandled |
| `ObjectSelect` | tell the sim which prims are selected | P4 | handled | sent before a delete because that is what a viewer does. The sim does not log it as unhandled, but nothing depends on it and nothing has confirmed it acted |
