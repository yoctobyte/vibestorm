...
from vibestorm.viewer.scene import (
...
class SceneEventApplicationTests(unittest.TestCase):
    def test_apply_region_changed_clears_markers_and_tile(self) -> None:
        scene = Scene(
            object_markers={1: Marker(1, PCODE_PRIM, (0, 0, 0), (1, 1, 1), 0.0)},
            avatar_markers={2: Marker(2, PCODE_AVATAR, (0, 0, 0), (1, 1, 1), 0.0)},
        ...
        self.assertEqual(scene.object_markers, {})
        self.assertEqual(scene.avatar_markers, {})
    ...
    def test_apply_chat_im_alert_outbound(self) -> None:
        scene = Scene()
        scene.apply_chat_im(
            ChatIM(region_handle=0, from_agent_name="Bob", to_agent_id=UUID(int=1), message="yo", dialog=0)
        )
...
class SceneWorldViewRefreshTests(unittest.TestCase):
    def test_refresh_with_none_world_view_clears_markers(self) -> None:
        scene = Scene(
            object_markers={1: Marker(1, PCODE_PRIM, (0, 0, 0), (1, 1, 1), 0.0)},
        )
    ...
    def test_refresh_categorizes_objects_by_pcode(self) -> None:
        ...
        self.assertIn(10, scene.object_markers)
        self.assertIn(20, scene.avatar_markers)
        self.assertNotIn(20, scene.object_markers)
    ...
    def test_refresh_uses_terse_for_avatars_without_full_update(self) -> None:
        ...
        self.assertIn(42, scene.avatar_markers)
        self.assertEqual(scene.avatar_markers[42].position, (20.0, 30.0, 25.0))
    ...
    def test_refresh_prefers_full_object_over_terse(self) -> None:
        ...
        # Full update wins; terse is ignored.
        self.assertEqual(scene.object_markers[99].position, (10.0, 10.0, 10.0))
        self.assertEqual(scene.object_markers[99].scale, (2.0, 3.0, 4.0))
...
class MarkerColorTests(unittest.TestCase):
    def test_known_pcode_uses_palette(self) -> None:
        m = Marker(local_id=1, pcode=PCODE_AVATAR, position=(0, 0, 0), scale=(1, 1, 1), rotation_z_radians=0)
        self.assertEqual(m.color, (255, 200, 80))

    def test_unknown_pcode_falls_back(self) -> None:
        m = Marker(local_id=1, pcode=200, position=(0, 0, 0), scale=(1, 1, 1), rotation_z_radians=0)
        self.assertEqual(m.color, DEFAULT_MARKER_COLOR)
